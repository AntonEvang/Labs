...
{
	int k;
	cout << "Введите год" << endl; cin >> k;
	while (k < 1984)
	{
		k += 60;
	}
	int delta = abs(k - 1984);
	int a = delta % 60;
	int rem = a % 12;
	string Animal[12] = { "крыса", "корова", "тигр", "заяц", "дракон", "змея", "лошадь", "овца", "обезьяна", "курица", "собака", "свинья" };
	int g = (a % 10) / 2;
	string Colour[5] = { "зеленый ", "красный ", "желтый ", "белый ", "черный "};
	cout << "год " << Colour[g] << Animal[rem] << endl;
	return 0;
}
